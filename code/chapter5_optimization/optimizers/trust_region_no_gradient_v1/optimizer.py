"""Expanding trust-region optimizer without the gradient direction."""
...
OPTIMIZER_ID = "trust_region_no_gradient_v1"
...
class TrustRegionNoGradient:
    ...
    def optimize(self, start: BenchmarkStart, ctx: OptimizationContext) -> OptimizationResult:
        ...
        for radius in RADII:
            dirs: list[np.ndarray] = list(axes)
        ...
        return finalize_result(
            ...
            selected=selected,
            status=status,
            stop_reason=status if status != "no_surrogate_crossing" else "trust_region_no_gradient_exhausted",
            start_probability=start_prob,
        )
